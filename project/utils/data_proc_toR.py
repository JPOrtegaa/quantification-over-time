...
def proc_single(val, test):
    val["TRAININGSET"] = 1
    test["TRAININGSET"] = 0
    comb = pd.concat([val, test], axis=0).rename(
        columns={"label": "TRUTH", "text": "TEXT"}
    )
    return comb


text_senti_data = [
    ("global_covid19_tweets", 15),
    ("nepali_dataset_eng", 15),
    ("Apple-Twitter-Sentiment-DFE", 15),
]

for data_name in text_senti_data:
    ts_chunks, ts_prevalence, c, t_size = data_loading.loading(data_name[0])
    val_set, test_sets, test_dsts = val_test_split(
        ts_chunks.copy(), ts_prevalence, data_name[1]
    )
