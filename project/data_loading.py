...
def nepali_dataset_eng():
    ...
    for i in range(len(dates)):
        ...
        pos_num = df1[(df1["Datetime"] == dates[i]) & (df1["label"] == 1)][
            "Datetime"
        ].count()
    ...
    neg_nums = []
    for i in range(len(dates)):
        neg_num = df1[(df1["Datetime"] == dates[i]) & (df1["label"] == -1)][
            "Datetime"
        ].count()
...
def global_covid19_tweets():
    ...
    df1["TweetAt"] = (
        df1["TweetAt"].str.split("-").str[1]
        + "-"
        + df1["TweetAt"].str.split("-").str[0]
    )
    ...
    for i in range(len(dates)):
        ...
        pos_num = df1[(df1["TweetAt"] == dates[i]) & (df1["label"] == 1)][
            "TweetAt"
        ].count()
        pos_nums.append(pos_num)
    ...
    neg_nums = []
    for i in range(len(dates)):
        neg_num = df1[(df1["TweetAt"] == dates[i]) & (df1["label"] == -1)][
            "TweetAt"
        ].count()
        neg_nums.append(neg_num)
...
def bike():
    ...
    for i, t in enumerate(timestamps):
        ...
        pos_num = df_date_index[
            (df_date_index["dteday"] == t) & (df_date_index["label"] == 1)
        ]["dteday"].count()
        pos_nums.append(pos_num)
    ...
    training_data, ts_data_dict, ts_prevalence = val_test_split(
        data_dict.copy(), prevalence_df, training_size
    )
...
def news():
    ...
    for i, t in enumerate(timestamps):
        ...
        pos_num = df_date_index[
            (df_date_index["timedelta"] == t) & (df_date_index["label"] == 1)
        ]["timedelta"].count()
        pos_nums.append(pos_num)
    pos_nums = np.array(pos_nums)
    pos_prevs = pos_nums / amounts
