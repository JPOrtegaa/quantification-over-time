...
tot_res = pd.read_csv(pakdd_dir / "MAE results seed 1 (has kf ens).csv").iloc[:, :3]
...
df_win_counts.to_csv(pakdd_dir / "win_counts.csv")
    ...
    for m_j in range(m_i - 1, -1, -1):
        ...
        for row in range(len(TSF_results)):
            if TSF_results.iloc[row, m_i] > TSF_results.iloc[row, m_j]:
                ...
                pair_win_counts[TSF[m_j]] = pair_win_counts[TSF[m_j]] + 1
        pairs.append(
            [TSF[m_i], TSF[m_j], pair_win_counts[TSF[m_i]], pair_win_counts[TSF[m_j]]]
        )
pairs = np.array(pairs)
pair_wins = pd.DataFrame(
    {
        "method 1": pairs[:, 0],
        "method 2": pairs[:, 1],
...
pair_wins.to_csv(pakdd_dir / "pair_wins.csv")

tot_res["best_method"] = np.array(best_m)
tot_res.to_csv(pakdd_dir / "MAE quanti_results mean.csv")
